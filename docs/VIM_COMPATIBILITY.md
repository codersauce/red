...
| Inline assist selection | **intentional difference** | `Space i` targets the complete current line in Normal mode or the exact character/linewise Visual selection. Visual-block targets are rejected. The popup has its own Insert-like single-line prompt, and its applied result is one unsaved, undoable editor transaction. |
