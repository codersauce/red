...
| Ex shell commands | **supported** | `:!{command}` executes the exact command through the user's non-interactive shell in Red's working directory, with asynchronously streamed stdout/stderr, bounded Messages history, cancellation with `Ctrl-c`, detached-session continuity, `%`/`#` filename expansion, escaped special characters, and previous-command expansion including `:!!`. Interactive terminal programs, `:read !`, `:write !`, and line-range filters such as `:%!sort` are not supported. |
