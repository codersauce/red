...
pub(crate) fn parse_csi_special_key_code(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    ...
    // xterm modifyOtherKeys uses CSI 27 ; modifiers ; codepoint ~.
    // Reuse the CSI-u decoder so Enter, Tab, Unicode, and modifiers have the
    // same representation regardless of which extended protocol was used.
    if first == 27 {
        let modifiers = next_parsed::<u8>(&mut split)?;
        let codepoint = next_parsed::<u32>(&mut split)?;
        if modifiers == 0 || split.next().is_some() {
            return Err(could_not_parse_event_error());
        }
        return parse_csi_u_encoded_key_code(
            format!("\x1b[{codepoint};{modifiers}u").as_bytes(),
        );
    }
...
mod tests {
    ...
    fn red_modified_enter_protocols() {
        for (sequence, modifiers) in [
            ...
            (&b"\x1b\r"[..], KeyModifiers::ALT),
            (&b"\x1b[13;2u"[..], KeyModifiers::SHIFT),
            (&b"\x1b[13;3u"[..], KeyModifiers::ALT),
            (&b"\x1b[13;5u"[..], KeyModifiers::CONTROL),
            (&b"\x1b[27;2;13~"[..], KeyModifiers::SHIFT),
            (&b"\x1b[27;3;13~"[..], KeyModifiers::ALT),
            (&b"\x1b[27;5;13~"[..], KeyModifiers::CONTROL),
        ] {
            assert_eq!(
                parse_event(sequence, false).unwrap(),
                Some(InternalEvent::Event(Event::Key(KeyEvent::new(
    ...
    #[test]
    fn red_modify_other_keys_uses_the_existing_character_decoder() {
        assert_eq!(
            parse_event(b"\x1b[27;3;120~", false).unwrap(),
            Some(InternalEvent::Event(Event::Key(KeyEvent::new(
                KeyCode::Char('x'),
                KeyModifiers::ALT,
            ))))
        ...
        for sequence in [
            ...
            assert!(parse_event(sequence, false).is_err(), "{sequence:?}");
        }
    }
