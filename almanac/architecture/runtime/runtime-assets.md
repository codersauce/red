...
`RuntimeAssetKind` owns the category boundary: plugin assets live under `plugins` and must be direct `.hk` files, while theme assets live under `themes` and must be direct `.json` files [@asset-system]. Asset names are normalized through `safe_relative_path`, which trims a leading slash, rejects empty paths, and rejects parent directories, roots, and platform prefixes [@asset-system]. Ejection adds one more constraint: explicit paths must start with `plugins/` or `themes/`, and the target file must be directly inside that category directory [@asset-system].
