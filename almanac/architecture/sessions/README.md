...
Read [Crash Recovery Snapshots](crash-recovery-snapshots) for persisted recovery. `SessionSnapshot` stores schema version, generation, working directory, buffers, window layout, registers, jumps, marks, last visual selections, agent transcript, proposal workspace snapshot, and resumability flags; `SessionStore` writes and loads `latest.json` and `previous.json` generations under owner namespaces [@session].
