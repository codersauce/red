...
`Editor` is documented as the single-task owner of Red's application state, and its fields group the major runtime domains: `BufferManager`, `WindowManager`, `SessionManager`, `LspCoordinator`, `AgentManager`, plugin registry, render buffers, diagnostics, panels, overlays, and dialogs [@editor]. The sub-managers keep domain state local without changing ownership. `AgentManager` stores the Codex bridge, task handle, proposal workspace, tool request receiver, active sessions, and turn timing [@agent-manager]. `LspCoordinator` tracks which document URIs have been opened and which buffer revisions have already been delivered to language servers [@lsp-coordinator]. `SessionManager` tracks the active session store, snapshot interval, in-flight writer, persisted generation, and warning state [@session-manager].
...
`service_background` is the loop's multiplexing point. It first drains a bounded number of agent editor tool requests and sends each result back on the tool response channel [@editor]. It then polls plugin timer callbacks, plugin process events, directory watcher changes, and plugin hot reloads before draining Codex bridge events into plugin notifications such as `agent:update`, `agent:completed`, and `agent:proposals_changed` [@editor]. The method also clears finished Codex tasks and emits `agent:session_lost` when the bridge has stopped and no pending events remain [@editor].
