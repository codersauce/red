...
Tree-sitter supplies UTF-8 byte ranges, which are translated through the rope into Red's Unicode-scalar `TextPosition` coordinates before operators or selections consume them [@buffer] [@textobjects]. Repeated captures in one query match are aggregated into a single enclosing range, preserving complete function and class interiors. Missing comment interiors and Fish function bodies are synthesized without evaluating Neovim-only directives [@textobjects].

Lookups prefer the smallest enclosing capture and can look ahead to the next object; directional motions remain strict and do not wrap. Parameter and function swaps require matching parent containers, apply both replacements through one editor transaction, and keep ownership of registers, marks, undo, repeat, macros, and jump history in the editor [@textobjects] [@editor]. Full-document parsing rejects files over 2 MiB, caps cached documents and captures, uses independent parse and query budgets, and reuses successful parses even when a subsequent object search fails [@textobjects]. Structural queries do not traverse Markdown-injected languages: only the document's selected top-level grammar participates [@highlighter] [@textobjects].

Default keymaps place structural call, function, and class motions in the existing `[` and `]` groups beside matchit, diagnostics, and Git hunk motions [@default-config]. `Space n` and `Space p` remain leaf mappings for next and previous buffer, so parameter and function swaps use the independent `Space ] a`, `Space [ a`, `Space ] m`, and `Space [ m` prefixes without changing buffer navigation or prefix resolution [@default-config].
