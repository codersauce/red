...
Agent requests share the same queue. Runtime actions enqueue `AgentNewSession`, prompts, cancellation, session close/archive, proposal acceptance or rejection, and permission responses [@runtime]. The editor owns the `AgentManager`, validates bridge and workspace state, starts Codex app-server tasks when needed, applies accepted proposals through editor transactions, and emits agent plugin notifications from the serialized loop [@editor].
