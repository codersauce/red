...
Application is one editor transaction. Red converts the rebased main LSP text edit or label/insert text into an editor range, expands snippet insert text into plain source plus numbered placeholder ranges, converts additional edits, sorts edits in descending position order, applies them, inserts any commit character, notifies LSP of the change, and commits the transaction [@editor]. Snippet placeholders become editor-owned Unicode-scalar edit anchors, so changing an earlier argument adjusts later argument positions through the same mutation boundary used by marks. The first placeholder is highlighted without entering Visual mode; typing or pasting replaces its default text, `Tab` and `Shift-Tab` navigate numbered placeholders, and the final `Tab` moves to `$0`. Leaving Insert mode, undoing, changing buffers, or moving outside the snippet ends its session [@editor]. When an item has no LSP text edit, the editor uses the active completion range instead of plain insertion, so accepting a label or `insertText` replaces the typed identifier prefix rather than appending after it [@editor]. If the completion item carries an LSP command, Red sends `workspace/executeCommand` after the edits have been applied [@editor]. Tests enforce interactive snippet navigation, atomic edits, and command execution after completion [@completion-tests].
