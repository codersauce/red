...
The full agent's non-LSP tools include `list_files`, `search_files`, `read_file`, `write_file`, `create_directory`, `get_editor_state`, `open_file`, `select_text`, `apply_edits`, `add_annotations`, `dismiss_annotations`, and `run_editor_action` [@workflow] [@codex]. The same editor-tool surface also exposes LSP status, diagnostics, rename preview, and apply-edit plan tools; [Dynamic Tools And Editor Tools](dynamic-tools-and-editor-tools) is the full inventory [@tools]. `list_files` and `read_file` are paged and report continuation or truncation metadata. `read_file` returns current editor-visible contents and revision, including unsaved buffer contents; a later page can be a fresh read, while a same-snapshot continuation must present the first page's revision [@codex] [@tools]. `write_file`, `apply_edits`, and `add_annotations` reuse the revision from the read they are based on [@codex] [@tools].
