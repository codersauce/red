...
The low-level parser in `src/command.rs` receives the authoritative built-in command list from its caller. It parses a trailing `!` as `CommandFlag::Force`, splits arguments on spaces without shell quoting, resolves exact command names and conventional initial-based abbreviations such as `bn` for `buffer-next`, and explicitly recognizes the `wq` save-and-quit chain [@command-parser]. Other unknown names are rejected instead of being decomposed into unrelated commands, preventing accidental buffer deletion, reloads, or writes [@command-parser].
