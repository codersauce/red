...
For this codebase, target the current host API unless there is a clear compatibility reason not to. `src/plugin/host_api.json` declares version `0.7.0`, and the registry accepts `0.4.0`, `0.6.0`, and the current `0.7.0` version for existing packages [@schema] [@registry]. New metadata should use `"red_api_version": "^0.7.0"` unless it intentionally needs to stay compatible with an older Red host API; keep `"^0.6.0"` only when the plugin avoids newer `0.7.0` features such as command metadata scope, and keep `"^0.4.0"` only when it also avoids newer `0.6.0` calls such as `CompanionCall`, `DocumentSnapshot`, `DocumentApply`, `DocumentUndo`, and `UpdatePickerBusy` [@schema] [@registry]. Use [Plugin host API](../../reference/plugins/host-api) for exact schema lookup and [Red host API](../../architecture/plugins/red-host-api) for how validation and dispatch work.
