...
| `src/plugin/registry.rs` | Runtime compatibility gate; `RED_HOST_API_VERSION` is `0.9.0`, and `0.4.0`, `0.6.0`, `0.7.0`, and `0.8.0` remain accepted for existing packages [@registry]. |
...
The current host API version is `0.9.0` [@schema] [@registry]. The schema marks `AgentResumeSession`, `AgentForgetSession`, and `UpdateOverlayBusy` as `0.9.0` calls, and the `OpenConfirm` signature now accepts an optional `options?: Json` argument [@schema]. The change manifest also records the agent conversation-restore events `agent:conversation_restore_pending`, `agent:session_restored`, and `agent:session_restore_failed`, plus `OpenConfirm.options`, as `0.9.0` introductions [@changes].
