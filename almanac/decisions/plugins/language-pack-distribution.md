...
Catalog UI states should keep expected incompatibilities recoverable. The 2026-08-04 review of the catalog branch identified Red API mismatches and missing host-target artifacts as normal picker states, not failed editor actions: selecting such a row should report the precise reason and keep the language-pack picker open instead of entering the install flow [@language-pack-discussion]. This keeps catalog discovery useful on every host even when only some packages are installable on that host.

Native grammar consent must be bound to the package and artifact the user actually reviewed. The same review found that refetching a mutable catalog after the consent picker can approve native grammar bytes whose digests were not shown to the user; catalog install actions therefore need to carry the already-confirmed catalog URL and resolved package forward, or re-prompt if a later fetch changes the shown artifact URL, size, archive SHA-256, or per-grammar digests [@language-pack-discussion]. Downloaded bytes still need checksum verification before activation, but verification is not a substitute for consent to the exact digests displayed [@package-code] [@language-pack-discussion].
