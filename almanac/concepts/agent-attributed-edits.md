...
The full Agent workflow starts from `Space A` or `:Agent` and uses the seventeen-tool Codex surface documented in the workflow, including workspace discovery, Red-mediated reads and writes, annotations, safe editor actions, and structured LSP diagnostics and rename tools [@workflow] [@tools]. Successful full-agent edits are revision-checked, applied as agent-origin transactions, and saved to disk through Red, while directory creation is workspace-confined and does not change an editor buffer and LSP edit application deliberately leaves changed buffers unsaved for review [@workflow] [@editor] [@tools].
