...
pub(super) fn apply_workspace_settings(
    ...
    if language_id != "rust" || !is_rust_analyzer(config) {
        return;
    }
...
/// Keep rust-analyzer responsive unless explicit configuration requests eager priming.
...
/// Recognize direct and rustup launches without guessing what arbitrary wrappers run.
fn is_rust_analyzer(config: &LanguageServerConfig) -> bool {
...
mod tests {
    ...
    #[test]
    fn rust_glancer_example_keeps_its_own_initialization_options() {
        let loaded = crate::config::Config::load_user_toml(
        ...
        assert!(loaded.config.lsp.servers.contains_key("husk"));
        let mut config = loaded.config.lsp.servers["rust"].clone();
        assert_eq!(config.command, "rust-glancer");
        assert_eq!(config.args, ["lsp"]);
        assert_eq!(config.documents()[0].language_id, "rust");
        assert_eq!(config.documents()[0].file_extensions, ["rs"]);
        assert_eq!(
            config.initialization_options,
            Some(json!({"diagnostics": {
                "onStartup": false, "onSave": true, "command": "check"
            }}))
        );
    ...
    #[test]
    fn rust_analyzer_launches_receive_automatic_settings() {
        let (repository, workspace) = repository();
        write_settings(
            repository.path(),
            r#"{"rust-analyzer.rustfmt.extraArgs":["--edition","2024"]}"#,
        );
        let absolute = repository.path().join("rust-analyzer");
        ...
        for (command, args) in [
            ...
            (
                ...
                vec!["run", "--install", "stable", "rust-analyzer.exe"],
            ),
            (
                "rustup",
                vec!["run", "stable", "--", absolute.to_str().unwrap(), "-v"],
            ),
        ] {
    ...
    fn other_rust_servers_preserve_explicit_settings() {
        ...
        for (command, args) in [
            ...
            config.command = command.to_string();
            config.args = args.into_iter().map(str::to_string).collect();
            config.initialization_options = Some(json!({"custom": true}));
            config.settings = Some(json!({"custom": {"enabled": true}}));
            let expected = config.clone();
            apply_workspace_settings(&mut config, &workspace, "rust");
            apply_fast_startup_defaults(&mut config, "rust");
            assert_eq!(config, expected);
