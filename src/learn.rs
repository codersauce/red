...
impl Lesson {
    ...
    pub const fn is_git_practice(self) -> bool {
        matches!(
            self,
            Self::ReviewWhatChanged | Self::StageTheRightHunk | Self::MakeALocalCommit
        )
    }
    ...
    pub const fn checkpoints(self) -> &'static [&'static str] {
        match self {
            ...
            Self::RepeatAndRecover => &[
                ...
                "Repeat it on the next line",
                "Undo only the repeated edit",
                "Redo the repeated edit",
            ],
            Self::ChangeATextObject => &[
                "Move inside the quoted title",
                "Change inside the quotes",
                "Type the new title",
                "Return to Normal mode",
            ],
            Self::MoveWithIntent => &[
            ...
            Self::OpenAFileByName => &[
                ...
                "Find the scoring source",
                "Find the matching test file",
                "Return to the project guide",
            ],
            Self::MakeALocalCommit => &[
                "Open a commit message",
                "Create the local commit",
                "Inspect the committed fix",
                "Return with the other change unstaged",
            ],
            Self::StageTheRightHunk => &[
...
impl PracticeStep {
    pub fn suggested_action(self) -> Option<Action> {
        match self {
            ...
            Self::RepeatRedo => Some(Action::Redo),
            Self::ObjectFind => Some(Action::FindCharForward {
                target: '"',
                count: 1,
            }),
            Self::ObjectChange | Self::ObjectType => None,
            Self::ObjectNormal => Some(Action::EnterMode(Mode::Normal)),
            ...
            Self::CommitWrite => Some(Action::Save),
            Self::CommitInspect | Self::CommitReturn => None,
    ...
    pub fn instruction(self, lesson: Lesson, shortcut: Option<&str>) -> String {
        match self {
            ...
            Self::RepeatRedo => format!("Press {} to redo that second edit. Repeat saves keystrokes, while undo and redo keep each completed change recoverable.", shortcut.unwrap_or("Ctrl-r")),
            Self::ObjectFind => r#"Move inside Old scoreboard on the first line. With the default keys, f" finds the opening quote and l moves inside it. A text object describes a region around the cursor."#.into(),
            Self::ObjectChange => r#"Type ci": change inside double quotes. The old title disappears, the quotes remain, and Red enters Insert mode. The same pattern works with words, parentheses, and other supported text objects."#.into(),
            Self::ObjectType => "Type Scoreboard. Only the text inside the quotes should change; keep the surrounding statement intact.".into(),
            Self::ObjectNormal => format!("Press {} to finish the change. Deleting the old title and typing its replacement form one undoable edit.", shortcut.unwrap_or("Esc")),
            ...
            Self::FilesReturn => format!("Press {}, search for README, and open README.md to return to the project guide. Each file remains an editor buffer while the lesson is active.", shortcut.unwrap_or("Ctrl-p")),
            Self::CommitOpen => format!("Only the score fix is staged. Open Git with {} (or :GitDashboard), press c, and choose Write message. This practice repository has no remote.", shortcut.unwrap_or("Space g g")),
            Self::CommitWrite => "Write a short message on the first line. Return to Normal mode, then use :w to create the local commit. :q cancels the message without committing. The commented diff is context, not part of your message.".into(),
            Self::CommitInspect => "The local commit exists. Press Tab to focus its diff, then move onto the changed score line. Check the short commit ID and verify that the title change was left out.".into(),
            Self::CommitReturn => "You inspected the committed arithmetic fix. Press q to return to code. The separate title change is still unstaged; nothing was pushed.".into(),
    ...
    pub fn observe(
        ...
        let next = match (*self, action) {
            ...
            (Self::RepeatNext, Action::RepeatLastChange)
                if contents == precision::REPEAT_RESULT =>
            {
                Self::RepeatUndo
            }
            (Self::RepeatUndo, Action::Undo) if contents == precision::REPEAT_FIRST => {
                Self::RepeatRedo
            }
            (Self::RepeatRedo, Action::Redo) if contents == precision::REPEAT_RESULT => {
                Self::Complete
            }
            (Self::ObjectFind, _)
                if original_text
                    && mode == Mode::Normal
                    && cursor.1 == 0
                    && (13..27).contains(&cursor.0) =>
            {
                Self::ObjectChange
            }
            (Self::ObjectChange, Action::ChangeTextRange(_))
                if mode == Mode::Insert && contents == precision::OBJECT_EMPTY =>
            {
                Self::ObjectType
            }
            (Self::ObjectType, _)
                if mode == Mode::Insert && contents == precision::OBJECT_RESULT =>
            {
                Self::ObjectNormal
            }
            (Self::ObjectNormal, Action::EnterMode(Mode::Normal))
                if mode == Mode::Normal && contents == precision::OBJECT_RESULT =>
            {
                Self::Complete
            }
    ...
    pub fn observe_view(&mut self, action: &Action, view: PracticeView) -> bool {
        let next = match (*self, action) {
            ...
            (Self::FilesTests, Action::OpenFile(_)) if view.navigation_tests_visible => {
                Self::FilesReturn
            }
            (Self::FilesReturn, Action::OpenFile(_)) if view.navigation_guide_visible => {
                Self::Complete
            }
...
pub(crate) struct PracticeView {
    ...
    pub navigation_guide_visible: bool,
