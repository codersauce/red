...
fn inline_input(prompt: &str, context: &str) -> Value {
    json!([{
        "type": "text",
        "text": format!(
            "Instruction:\n{prompt}\n\nEditor-owned target and context:\n{context}\n\nReturn by calling exactly one of submit_comments, submit_replacement, or request_agent.",
        )
    }])
}
